...
#### [NEW] Database Table (`kms_index_status`)
Tracks granular status for Snippets and Clipboard entities.
```sql
...
#### [MODIFY] `IndexingStatusDto` Structure
```rust
...
#### [NEW] API Commands
- `kms_get_indexing_details(provider_id: String) -> Vec<FailureDetailDTO>`: Returns list of failed items.
- `kms_retry_item(provider_id: String, entity_id: String)`: Retries a single item.
