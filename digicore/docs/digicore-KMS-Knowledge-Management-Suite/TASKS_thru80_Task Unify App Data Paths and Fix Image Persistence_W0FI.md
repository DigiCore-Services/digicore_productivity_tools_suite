...
- [x] PHASE 18: Feature Documentation
    ...
    - [x] Frontend: Restore premium UI (Badge, Skeleton, Toast, Tooltip)
    - [x] Frontend: Fix production build (tsc errors and import paths)
...
### PHASE 60: Surgical OCR Heuristic Tuning
- [x] Analyze false-positive table detection in `Example10_Plain-Text`
- [x] Implement stricter `min_avg_segments` and alignment checks in `windows_ocr.rs`
- [x] Verify `Example10` output is clean Markdown paragraphs (no pipes)
- [x] Ensure `Example3` (Simple Table) and `Example1` (Column Data) still work
- [x] Run full regression suite and verify dashboard accuracy improvements

### PHASE 61: OCR Summary UX Improvements
- [x] Add unique IDs to cards in `summary.html`
- [x] Update detail page "Back" links to use anchor hashes
- [x] Add smooth scroll and target highlight CSS to summary dashboard
- [x] Verify scroll restoration during manual review

### PHASE 62: Version Control & Remote Sync (Phases 12-61)
- [x] Group and stage changes for Commit 1: Core Domain Infrastructure
- [x] Group and stage changes for Commit 2: Layout-Aware OCR Engine
- [x] Group and stage changes for Commit 3: Regression Testing Framework
- [x] Group and stage changes for Commit 4: Tauri GUI Integration
- [x] Group and stage changes for Commit 5: Documentation & Architecture
- [x] Push all commits to remote branch `feature/ui-decoupling-phase-0-1`

### PHASE 63: Repository Cleanup & Maintenance
- [x] Identify and stage deletions of legacy `results_*` directories
- [x] Commit and push repository cleanup to remote

### PHASE 64: Refactor ConfigTab UX to Sub-Tabs
- [x] Implement `activeGroup` state in `ConfigTab.tsx`
- [x] Add sub-tab navigation button row to `ConfigTab.tsx`
- [x] Convert accordion sections to conditional rendering blocks
- [x] Verify UI responsiveness and save functionality

### PHASE 65: Persist Sub-Tab State
- [x] Implement `localStorage` persistence in `ConfigTab.tsx`
- [x] Implement `localStorage` persistence in `ScriptTab.tsx`
- [x] Verify state restoration across main tab navigation
- [x] Verify state restoration after app restart

### PHASE 66: Image Library & Layout OCR Viewer
- [x] Backend: Add `metadata` column (Migration #4) in `lib.rs`
- [x] Backend: Update `clipboard_repository.rs` to persist OCR metadata
- [x] Backend: Implement paged image gallery query in `clipboard_repository.rs`
- [x] Backend: Expose `get_image_gallery` and `get_child_entries` via TauRPC in `api.rs`
- [ ] Frontend: Create `ImageLibraryTab.tsx` with pagination and gallery grid
- [ ] Frontend: Create `ImageViewerModal.tsx` with OCR layout overlay and navigation
- [ ] Frontend: Integrate "Image Library" tab into main navigation
- [ ] Verification: Test pagination, search, and OCR overlay alignment
- [x] Phase 67: Build Warning Cleanup [2026-03-08]
    - [x] Resolve warnings in `corpus_generator.rs`
    - [x] Resolve warnings in `windows_ocr.rs`
    - [x] Verify clean build lux Done.

- [x] Phase 68: Persistent Crash Recovery
    - [x] Restore SQL migrations 2 and 3 in `lib.rs` to fix checksum panic
    - [x] Implement "Lightweight Startup" by deferring heavy tasks in `setup()`
    - [x] Verify app starts without crash 101 or freezing
    - [x] Restore clipboard entry observer and diagnostic logging
    - [x] Verify clipboard history updates and syncs correctly
 lux Phase 68 crash fix. lux Done.
- [x] Phase 69: Image Library Restoration
    - [x] Restore "Image Library" tab in `App.tsx` navigation and rendering
    - [x] Verify Image Library gallery displays images correctly
    - [x] Verify ImageViewerModal integration from Image Library
 lux Done.
- [x] Phase 70: UI Stability & Modal Fixes
    - [x] Fix React hook violation in `ImageViewerModal.tsx` to prevent crash on open/navigate
    - [x] Restored missing event handlers in `ImageViewerModal.tsx`
    - [x] Update `sqliteSync.ts` with `INSERT OR REPLACE` to prevent UNIQUE constraint errors
    - [x] Verify ImageViewerModal opens correctly from Image Library
    - [x] Verify startup console is free of SQL UNIQUE constraint errors
- [x] Phase 71: Image Library Context Menu Parity
    - [x] Draft implementation plan for native context menu parity
    - [x] Implement `showNativeContextMenu` in `ImageLibraryTab.tsx`
    - [x] Align actions (Open, Save, Copy, Delete) with Clipboard History tab
    - [x] Verify right-click menu functionality in Gallery view
- [x] Phase 72: ImageViewerModal Context Menu Parity
    - [x] Draft implementation plan for native context menu parity
    - [x] Implement `showNativeContextMenu` in `ImageViewerModal.tsx`
    - [x] Align actions (Save, Copy, Delete) with rest of the app
    - [x] Verify right-click menu functionality in Image Viewer
- [x] Phase 73: Deletion Concurrency & UI Polish
    - [x] Draft implementation plan for async deletion confirm
    - [x] Switch to Tauri's native `ask` dialog in Image Library/Viewer
    - [x] Fix race condition causing premature success toasts
    - [x] Verify deletion confirmation blocks as expected
- [x] Phase 74: Cross-Component Deletion Sync
    - [x] Draft implementation plan for deletion sync
    - [x] Add `onDeleteSuccess` callback to `ImageViewerModal`
    - [x] Trigger gallery refresh in `ImageLibraryTab`
    - [x] Verify gallery scales back/refreshes after Viewer deletion
- [x] Phase 75: Knowledge Management Suite (Notes & Media) Planning
    - [x] Research modern local-first PKM (Personal Knowledge Management) architectures
    - [x] Draft Technical & Architectural Document in `docs/digicore-notes/` (Rev 2 - Future Proofing)
    - [x] Integrate single-DB source of truth and dual-mode editor decisions
    - [x] Identify foundational missing elements (Semantic Search, Block Model, Plugins)
- [x] Phase 76: KMS Foundation & Vault Initialization
    - [x] Create implementation plan for Foundation
    - [x] Update `digicore.db` schema (Migrations)
    - [x] Implement multi-window launch command for KMS
    - [x] Implement Vault directory initialization and watchdog
    - [x] Verify window launching and DB connectivity
- [x] Phase 77: KMS Hybrid Editor Core
    - [x] Create implementation plan for Hybrid Editor
    - [x] Implement TipTap WYSIWYG integration
    - [x] Implement CodeMirror Source View integration
    - [x] Implement basic FS Save/Load logic
    - [x] Implement Note Renaming (Backend & Frontend)
    - [x] Integrate Tailwind Typography for Markdown rendering
    - [x] Resolve Vite 500 Internal Server Error & CSS variable sync
    - [x] Fix missing headers in Visual mode (prose-invert fix)
    - [x] Implement Theme Override (Global Sync & Local Toggle)
- [x] Phase 78: KMS Semantic Search & Vector Integration
    - [x] Integrate `sqlite-vec` extension
    - [x] Implement local embedding generation service
    - [x] Add unified search backend commands
    - [x] Frontend: KMS Search UI with logic/regex support
    - [x] Frontend: mandatory "Similar Notes" widget in KmsEditor
- [x] Phase 78b: KMS Hybrid Search Development
    - [x] Create Migration 6 for FTS5 table and triggers in `lib.rs`
    - [x] Refactor `kms_repository.rs` to implement FTS5 text search and Reciprocal Rank Fusion (RRF)
    - [x] Run `cargo check` & `tauri dev` to apply migration and verify build
- [/] Phase 78c: KMS Database Resilience & Surgical Recovery
    - [x] Implement `kms_repair_database` backend command (Drops FTS/Vector/KMS tables)
    - [ ] Create surgical migration reset logic (Clear `__tauri_migrations` for v4-v7)
    - [ ] Frontend: Add "Repair KMS Index" button in Vault Settings
    - [ ] Verify surgical recovery without affecting Snippets/Clipboard tables
- [ ] Phase 79: KMS Link Graph & Bi-directional Logic
    - [ ] Implement `[[Link]]` parsing and indexing
    - [ ] Build Graph visualization (Canvas/D3)
- [ ] Phase 80: KMS Media & Bookmark Plugins
    - [ ] Implement Bookmark retrieval service
    - [ ] Build Media Gallery integration in Notes
