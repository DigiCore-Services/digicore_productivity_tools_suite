...
- [ ] Backend: Add `metadata` column (Migration #4) in `lib.rs`
- [ ] Backend: Update `clipboard_repository.rs` to persist OCR metadata
- [ ] Backend: Implement paged image gallery query in `clipboard_repository.rs`
- [ ] Backend: Expose `get_image_gallery` and `get_image_ocr_details` via TauRPC in `api.rs`
