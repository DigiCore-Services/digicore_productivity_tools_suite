...
- **Interactive Icon**: A 🖼️ icon appears next to OCR entries; click it to open the parent source image.
- **Action Column**: A dedicated "Image" button in the Actions column for immediate access to the source.
- **Context Menu**: "View Source Image" available directly from the right-click menu.
- **Promote to Snippet**: OCR text can be directly converted into a reusable snippet with one click.
