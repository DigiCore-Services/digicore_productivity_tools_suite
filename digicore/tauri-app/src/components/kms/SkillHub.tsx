...
import KmsHealthDashboard from "./KmsHealthDashboard";
...
export default function SkillHub({ onSelectSkill, onCreateNew, refreshKey = 0 }: SkillHubProps) {
    ...
    const [activeTab, setActiveTab] = useState<"skills" | "health">("skills");
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-dc-text to-dc-text/50 bg-clip-text text-transparent">
                            {activeTab === "skills" ? "Skill Hub" : "System Health"}
                        </h1>
                    </div>
                    <p className="text-dc-text-muted text-sm max-w-lg">
                        {activeTab === "skills"
                            ...
                            : "Monitor indexing status, vector counts, and repository health. Perform maintenance tasks like history pruning and deep reindexing."}
                    </p>
                </div>

                <div className="flex gap-3">
                    <div className="flex bg-dc-bg-secondary/40 backdrop-blur-md rounded-xl p-1 border border-dc-border/50 mr-2">
                        <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setActiveTab("skills")}
                            className={`h-8 px-4 text-xs font-bold transition-all rounded-lg ${activeTab === "skills" ? "bg-dc-accent text-white shadow-lg" : "text-dc-text-muted hover:text-dc-text"}`}
                        >
                            Skills
                        </Button>
                        <Button
                            ...
                            className={`h-8 px-4 text-xs font-bold transition-all rounded-lg ${activeTab === "health" ? "bg-dc-accent text-white shadow-lg" : "text-dc-text-muted hover:text-dc-text"}`}
                        >
                            Health
                        </Button>
                    </div>
                ...
                <AnimatePresence mode="wait">
                    {activeTab === "skills" ? (
                        <motion.div
                            ...
                            className="space-y-8"
                        >
                            {/* toolbar copy within animated div */}
